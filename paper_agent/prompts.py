...
你的名字叫小智，是一名专业、严谨的学术论文审稿人和研究助理。
你的语言可以幽默风趣、亲切自然，但结论必须专业、具体、可追溯。
...
def chunk_review_prompt(chunk_index: int, total_chunks: int, paper_text: str) -> str:
    return f"""\
下面是一篇论文的第 {chunk_index}/{total_chunks} 个片段。请先做局部审稿笔记。
...
def paper_card_prompt(paper_name: str, notes: list[str]) -> str:
    ...
    return f"""\
请基于以下审稿笔记，为论文《{paper_name}》生成结构化 paper_card。
...
def section_review_prompt(paper_name: str, section_title: str, paper_card: str, notes: list[str]) -> str:
    ...
    return f"""\
请为论文《{paper_name}》撰写 Markdown 总结中的"{section_title}"一节。
...
def full_report_prompt(paper_name: str, paper_card: str, notes: list[str]) -> str:
    """Generate a prompt that produces all 4 report sections in a single LLM call."""
    joined_notes = "\n\n".join(
        f"===== 审稿笔记 {index} =====\n{note}"
        for index, note in enumerate(notes, start=1)
    )
    return f"""\
请基于 paper_card 和审稿笔记，为论文《{paper_name}》撰写完整的审稿式阅读总结。

严格按以下四个一级标题组织，每个标题用 # 开头：

# 摘要
概括论文研究问题、方法、结论和价值。用 1-3 段，信息要完整。

# 主要内容
说明研究背景、问题定义、整体思路、主要贡献和章节逻辑。

# 核心算法
说明核心模型、算法流程、关键公式、输入输出和技术动机。如果不是算法型论文，总结核心方法论。

# 算例分析
说明实验/算例设置、评价指标、对比方法、主要结果，以及结果如何支撑结论。
...
def compact_notes_prompt(batch_index: int, total_batches: int, notes: list[str]) -> str:
    ...
    return f"""\
下面是论文分片审稿笔记的第 {batch_index}/{total_batches} 批。请压缩为一份中间审稿摘要。
